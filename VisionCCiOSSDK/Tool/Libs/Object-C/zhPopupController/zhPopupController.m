...
- (void)presentDuration:(NSTimeInterval)duration
    ...
    __block void (^finishedCallback)(void) = ^() {
        ...
        if (weakSelf.didPresentBlock) {
            weakSelf.didPresentBlock(weakSelf);
        } else {
            if ([weakSelf.delegate respondsToSelector:@selector(popupControllerDidPresent:)]) {
                [weakSelf.delegate popupControllerDidPresent:weakSelf];
            }
        }
        
        if (weakSelf.dismissAfterDelay > 0) {
            self->_timer = [NSTimer timerWithTimeInterval:weakSelf.dismissAfterDelay target:weakSelf selector:@selector(timerPerform) userInfo:nil repeats:NO];
