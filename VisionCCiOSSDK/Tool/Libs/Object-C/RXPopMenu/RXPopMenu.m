...
#else
#define WEAK_SELF(type) __block typeof(type) weakSelf=self;
#endif
...
@synthesize cseparatorInset = _cseparatorInset;
...
- (UITableView *)getShowTarget{
    return self.popTableView;
}

- (UICollectionView *)getMenuTarget {
    return self.popCollectionView;
}
    ...
    if (CGSizeEqualToSize(CGSizeZero, _menuSize)) {
        switch (self.menuType) {
            case RXPopMenuList: {
                ...
                __block RXPopMenuItem * maxItem;
                [_items enumerateObjectsUsingBlock:^(RXPopMenuItem * _Nonnull item, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (UIEdgeInsets)cseparatorInset {
    return _cseparatorInset;
}
...
- (UITableView *)popTableView {
    if (!_popTableView) {
        ...
        _popTableView.delegate = weakSelf;
        _popTableView.dataSource = weakSelf;
        
        //适配平板
        _popTableView.cellLayoutMarginsFollowReadableWidth = NO;
        
        //分割线
        _popTableView.separatorStyle = UITableViewCellSeparatorStyleSingleLine;
        _popTableView.separatorInset = self.cseparatorInset;
        ...
        if (@available(iOS 13.0, *)) {
            _popTableView.automaticallyAdjustsScrollIndicatorInsets = NO;
        } else {
            // Fallback on earlier versions
        }
        
        if (@available(iOS 15.0, *)) {
            _popTableView.sectionHeaderTopPadding = 0;
        } else {
            // Fallback on earlier versions
        }
        
        //NSBundle *bundle = [NSBundle bundlewi]
        [_popTableView registerNib:[UINib nibWithNibName:RXPopMenuCellID bundle:[self bundleWithBundleName:@"RXPopMenu" podName:@"RXPopMenu"]]
...
- (UICollectionView *)popCollectionView {
    if (!_popCollectionView) {
        ...
        _popCollectionView.delegate = (id)weakSelf;
        _popCollectionView.dataSource = weakSelf;
        ...
        if (@available(iOS 13.0, *)) {
            _popCollectionView.automaticallyAdjustsScrollIndicatorInsets = NO;
        }
        ...
        //UINib *nib = [self nibForBuncleClass:[self class] andBundleName:@"RXPopMenu" withNibName:RXPopBoxCellID andNibFileName:nil];
        UINib *nib = [UINib nibWithNibName:RXPopBoxCellID bundle:[self bundleWithBundleName:@"RXPopMenu" podName:@"RXPopMenu"]];
        
        [_popCollectionView registerNib:nib forCellWithReuseIdentifier:RXPopBoxCellID];
    }
...
- (UIView *)popView {
    if (!_popView) {
        ...
        view.layer.shadowOffset = CGSizeMake(-frame.origin.x, RXSafeTop - frame.origin.y * 1.055);//CGSizeMake(0, 4);
        view.layer.shadowOpacity = 1;
        view.layer.shadowRadius = 10;
        
        UIBezierPath *path = [UIBezierPath bezierPathWithRect:frame];
        if(self.cornerRadius > 0) {
            path = [UIBezierPath bezierPathWithRoundedRect:frame cornerRadius:self.cornerRadius];
        }
...
- (CGRect)targetViewFrame {
    ...
    if (CGRectEqualToRect(_targetViewFrame, CGRectZero)) {
        ...
        if (self.inNaviBar) {
            ...
            /// 偏移适配 Add by wzq in 23/07/13
            if(self.visibleHeight != RXScreenHeight){
                CGFloat _h = RXScreenHeight - self.visibleHeight;
                targetFrame.origin.y -= _h;
            }
...
- (CGRect)getPopFrame {
    ...
    CGFloat spac = self.isShowSpac == NO ? 0.f : 10.f;
...
-(UIImage *_Nullable)imageForBuncleClass:(Class _Nullable)_bclass
                       ...
                       andImagesFileName:(NSString *_Nullable)_imgFileName {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    @autoreleasepool {
        ...
        cell.leftImageView.image = item.image ? [self imageForBuncleClass:[self class] andBundleName:@"RXPopMenu" withImageName:item.image andImagesFileName:@"images"] : nil;
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    @autoreleasepool {
        ...
        cell.topImageView.image = item.image ? [self imageForBuncleClass:[self class] andBundleName:@"RXPopMenu" withImageName:item.image andImagesFileName:@"images"] : nil;
        return cell;
    }
}
