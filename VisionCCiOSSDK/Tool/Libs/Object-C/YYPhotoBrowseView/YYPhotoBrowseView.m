...
- (void)layoutSubviews {
    ...
    _progressLayer.center = CGPointMake(self.vxiWidth / 2, self.vxiHeight / 2);
}
...
- (void)resizeSubviewSize {
    _imageContainerView.vxiOrigin = CGPointZero;
    _imageContainerView.vxiWidth = self.vxiWidth;
    
    UIImage *image = _imageView.image;
    if (image.size.height / image.size.width > self.vxiHeight / self.vxiWidth) {
        _imageContainerView.vxiHeight = floor(image.size.height / (image.size.width / self.vxiWidth));
    } else {
        CGFloat height = image.size.height / image.size.width * self.vxiWidth;
        if (height < 1 || isnan(height)) height = self.vxiHeight;
        height = floor(height);
        _imageContainerView.vxiHeight = height;
        _imageContainerView.vxiCenterY = self.vxiHeight / 2;
    }
    if (_imageContainerView.vxiHeight > self.vxiHeight && _imageContainerView.vxiHeight - self.vxiHeight <= 1) {
        _imageContainerView.vxiHeight = self.vxiHeight;
    }
    self.contentSize = CGSizeMake(self.vxiWidth, MAX(_imageContainerView.vxiHeight, self.vxiHeight));
    [self scrollRectToVisible:self.bounds animated:NO];
    
    if (_imageContainerView.vxiHeight <= self.vxiHeight) {
        self.alwaysBounceVertical = NO;
    } else {
...
- (instancetype)initWithGroupItems:(NSArray *)groupItems {
    ...
    _scrollView.frame = CGRectMake(-kPadding / 2, 0, self.vxiWidth + kPadding, self.vxiHeight);
    ...
    _pager.vxiWidth = self.vxiWidth - 36;
    _pager.vxiHeight = 10;
    _pager.center = CGPointMake(self.vxiWidth / 2, self.vxiHeight - 18);
...
- (void)presentFromImageView:(UIView *)fromView
    ...
    self.vxiSize = _toContainerView.vxiSize;
    ...
    _scrollView.contentSize = CGSizeMake(_scrollView.vxiWidth * self.groupItems.count, _scrollView.vxiHeight);
    [_scrollView scrollRectToVisible:CGRectMake(_scrollView.vxiWidth * _pager.currentPage, 0, _scrollView.vxiWidth, _scrollView.vxiHeight) animated:NO];
    ...
    if (item.thumbClippedToTop) {
        ...
        CGFloat scale = fromFrame.size.width / cell.imageContainerView.vxiWidth;
        
        cell.imageContainerView.vxiCenterX = CGRectGetMidX(fromFrame);
        cell.imageContainerView.vxiHeight = fromFrame.size.height / scale;
        cell.imageContainerView.layer.transformScale = scale;
        cell.imageContainerView.vxiCenterY = CGRectGetMidY(fromFrame);
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    [UIView animateWithDuration:animated ? 0.2 : 0 delay:0 options:UIViewAnimationOptionBeginFromCurrentState|UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        if (isFromImageClipped) {
            ...
            CGFloat scale = fromFrame.size.width / cell.imageContainerView.vxiWidth * cell.zoomScale;
            CGFloat height = fromFrame.size.height / fromFrame.size.width * cell.imageContainerView.vxiWidth;
            if (isnan(height)) height = cell.imageContainerView.vxiHeight;
            
            cell.imageContainerView.vxiHeight = height;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    CGFloat floatPage = _scrollView.contentOffset.x / _scrollView.vxiWidth;
    NSInteger page = _scrollView.contentOffset.x / _scrollView.vxiWidth + 0.5;
    ...
    for (NSInteger i = page - 1; i <= page + 1; i++) { // preload left and right cell
        if (i >= 0 && i < self.groupItems.count) {
            ...
            if (!cell) {
                ...
                cell.page = i;
                cell.vxiLeft = (self.vxiWidth + kPadding) * i + kPadding / 2;
...
- (void)updateCellsForReuse {
    for (YYPhotoGroupCell *cell in _cells) {
        if (cell.superview) {
            if (cell.vxiLeft > _scrollView.contentOffset.x + _scrollView.vxiWidth * 2||
                cell.vxiRight < _scrollView.contentOffset.x - _scrollView.vxiWidth) {
...
- (void)setShowFirstViewFor:(NSInteger)index {
    ...
    CGPoint pt = CGPointMake(_scrollView.vxiWidth * index, 0);
...
- (void)showHUD:(NSString *)msg {
    ...
    label.vxiSize = CGSizePixelCeil(size);
    label.font = font;
    label.text = msg;
    ...
    hud.vxiSize = CGSizeMake(label.vxiWidth + 20, label.vxiHeight + 20);
    hud.backgroundColor = [UIColor colorWithWhite:0.000 alpha:0.650];
    hud.clipsToBounds = YES;
    hud.layer.cornerRadius = 8;
    
    label.center = CGPointMake(hud.vxiWidth / 2, hud.vxiHeight / 2);
    [hud addSubview:label];
    
    hud.center = CGPointMake(self.vxiWidth / 2, self.vxiHeight / 2);
    hud.alpha = 0;
    [self addSubview:hud];
...
- (void)doubleTap:(UITapGestureRecognizer *)g {
    ...
    if (tile) {
        if (tile.zoomScale > 1) {
            ...
            CGFloat xsize = self.vxiWidth / newZoomScale;
            CGFloat ysize = self.vxiHeight / newZoomScale;
    ...
    switch (g.state) {
        ...
        case UIGestureRecognizerStateEnded: {
            ...
            if (fabs(v.y) > 1000 || fabs(deltaY) > 120) {
                ...
                CGFloat vy = fabs(v.y);
                if (vy < 1) vy = 1;
                CGFloat duration = (moveToTop ? _scrollView.vxiBottom : self.vxiHeight - _scrollView.vxiTop) / vy;
                duration *= 0.8;
                ...
                [UIView animateWithDuration:duration
                    ...
                    self->_pager.alpha = 0;
                    if (moveToTop) {
                        self->_scrollView.vxiBottom = 0;
                    } else {
                        self->_scrollView.vxiTop = self.vxiHeight;
                    }
                } completion:^(BOOL finished) {
                    [self removeFromSuperview];
                }];
 ...
 */
-(void) setVxiOrigin:(CGPoint)newOrigin
{
 ...
 */
-(void) setVxiSize:(CGSize)newSize
{
...
-(void) setVxiOriginX:(CGFloat)originX
{
...
-(void)setVxiOriginY:(CGFloat)originY
{
...
- (void)setVxiCenterX:(CGFloat)centerX {
...
- (void)setVxiCenterY:(CGFloat)centerY {
 ...
 *  右
 */
-(CGFloat)vxiFrameRight
{
    return self.frame.origin.x + self.frame.size.width;
}

-(void)setVxiFrameRight:(CGFloat)newRight
{
 ...
 *  底
 */
-(CGFloat)vxiFrameBottom
{
    return self.frame.origin.y + self.frame.size.height;
}

-(void)setVxiFrameBottom:(CGFloat)newBottom
{
 ...
 *  宽
 */
-(CGFloat)vxiFrameWidth
{
    return self.frame.size.width;
}

-(void)setVxiFrameWidth:(CGFloat)newWidth
{
...
- (void)setVxiWidth:(CGFloat)width{
 ...
 *  高
 */
-(CGFloat)vxiFrameHeight
{
    return self.frame.size.height;
}

-(void)setVxiFrameHeight:(CGFloat)newHeight
{
...
- (void)setVxiHeight:(CGFloat)height{
...
- (void)setVxiRight:(CGFloat)right {
    CGRect frame = self.frame;
...
- (void)setVxiBottom:(CGFloat)bottom {
    CGRect frame = self.frame;
