...
class TGSUIModel: NSObject {
    ...
    static func createDiyNavgationalViewFor(TitleStr _t: String,
        ...
        backBtn.rx.safeTap.subscribe {event in
            _bb?()
        }.disposed(by: _dispos)
    ...
    /// - Parameters:
    ///   - _url: String
    ///   - _showImg: YYAnimatedImageView
    ///   - _vs: CGSize
    ///   - _cornerRadius: CGFloat? 圆角
    ///   - _fc: CGColor? 填充颜色
    static func bindImageFor(Url _url:String,
                             andView _showImg:YYAnimatedImageView,
                             andViewSize _vs:CGSize,
                             andCornerRadius _cornerRadius:CGFloat? = nil,
                             withFillColor _fc:CGColor? = UIColor.init().colorFromHexInt(hex: 0x68A6F5).cgColor,
                             andContentOffset _offset:CGPoint? = nil,
                             andeDefaultImage _dfimg:UIImage? = VXIUIConfig.shareInstance.cellDefaultImage()){
        
        //先移除
        let _svg_tag = VXIUIConfig.shareInstance.getSVGTag()
        _showImg.subviews.first(where: { $0.tag == _svg_tag })?.removeFromSuperview()
        
        guard let _newUrl = URL.init(string: TGSUIModel.getFileRealUrlFor(Path: _url, andisThumbnail: true)) else { return }
        
        //MARK: SVG 矢量图
        if _url.lowercased().hasSuffix(".svg") || _url.lowercased().contains(".svg?") {
            debugPrint("bindImageFor:SVG 矢量图:\(_url)")
            let _tempBlock:((SVGLayer)->Void) = { _svgLayer in
                if _offset != nil {
                    _svgLayer.resizeToFit(.init(origin: _offset!,
                                                size: .init(width: _vs.width - 2 * _offset!.x,
                                                            height: _vs.height - 2 * _offset!.y)))
                }
                else{
                    _svgLayer.resizeToFit(.init(origin: .zero, size: _vs))
                }
            }
            
            if _url.hasPrefix("http"){
                VXIDownLoadFileManager.share.downloadFileInCacheFor(Url: _url,
                                                                    andTitle: _newUrl.lastPathComponent,
                                                                    andLoading: false,
                                                                    withProgressBlock: nil) {(_isOK, _data, _fileCachePath, _msg) in
                    if _isOK,let _d = _data {
                        let _imgView = SVGView.init(SVGData: _d) { (svgLayer) in
                            if _fc != nil {
                                svgLayer.fillColor = _fc!
                            }
                            
                            _tempBlock(svgLayer)
                        }
                        
                        _imgView.tag = _svg_tag
                        _showImg.image = nil
                        _showImg.addSubview(_imgView)
                    }
                    else{
                        debugPrint("bindImageFor:SVG 矢量图加载失败！详见：{path:\(_url),_fileCachePath:\(_fileCachePath ?? "--")}")
                    }
                }
            }
            else{
                let _imgView = SVGView.init(SVGNamed: TGSUIModel.getFileRealUrlFor(Path: _url, andisThumbnail: true)) { (svgLayer) in
                    if _fc != nil {
                        svgLayer.fillColor = _fc!
                    }
                    _tempBlock(svgLayer)
        ...
        //MARK: gif 动图
        else if _url.lowercased().hasSuffix(".gif") || _url.lowercased().contains(".gif?") {
            debugPrint("bindImageFor:gif 动图 \(_url)")
            if _url.hasPrefix("http") {
                _showImg.yy_setImage(with: _newUrl,
                                     placeholder: _dfimg,
                                     options: VXIUIConfig.shareInstance.requestOption())
            }
            else{
                _showImg.image = YYImage.init(named: _url)
            }
        }
        //MARK: 普通图片(标量图,jpg/jpeg/png/bmp...)
        else{
            if _url.hasPrefix("http") {
                debugPrint("bindImageFor:普通图片:\(_url)")
                
                //progressive 渐进式，边下载边加载
                //https://www.cnblogs.com/Camier-myNiuer/p/4958144.html
                YYWebImageManager.shared().requestImage(with: _newUrl,
                                                        options: VXIUIConfig.shareInstance.requestOption(),
                                                        progress: nil,
                                                        transform: nil) { (_img:UIImage?, _:URL, _:YYWebImageFromType, _:YYWebImageStage, _:Error?) in
                    DispatchQueue.main.async {
                        if _cornerRadius != nil && _cornerRadius! > 0 {
                            if _img != nil,let img = _img!.yy_imageByResize(to: _vs,
                                                                            contentMode:VXIUIConfig.shareInstance.cellImageContentMode())?.yy_image(byRoundCornerRadius:_cornerRadius!) {
                                _showImg.image = img
                            }
                        }
                        else{
