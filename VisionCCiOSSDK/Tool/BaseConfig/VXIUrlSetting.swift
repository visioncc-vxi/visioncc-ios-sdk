...
struct VXIUrlSetting {
    ...
    private let host:String = {
        let _host = UserDefaults.standard.string(forKey: VXIUIConfig.shareInstance.getHostKey()) ?? "https://vcc-sdk.vxish.cn"
        debugPrint("http(s) 域名配置:\(_host)")
        return _host
    }()
    
    /// 文件域名
    let fileHost:String = {
        let _file_host = (UserDefaults.standard.string(forKey: VXIUIConfig.shareInstance.getHostKey()) ?? "https://vcc-sdk.vxish.cn") + "/api/file"
        debugPrint("文件域名:\(_file_host)")
        return _file_host
    }()
    
    /// socketIO 地址
    func getSocketIOHost() -> String  {
        var _host = "vcc-sdk.vxish.cn"
        let _arr = host.components(separatedBy: "//")
        if _arr.count > 0,
           let _nh = _arr.last,_nh.isEmpty == false {
            _host = _nh
        }
        
        let _socketIO_url = "wss://\(_host)"
        debugPrint("socketIO:\(_socketIO_url)")
        return _socketIO_url
    }
