...
class  VXIDownLoadFileManager: NSObject {
    ...
    func downloadFileInCacheFor(Url _strUrl:String,
        ...
        var _p = VXIUIConfig.shareInstance.getCachePath() + "/\(_title.hashHex(by: .MD5))"
        let _attr = _title.components(separatedBy: ".")
        if _attr.count > 0 {
            _p = VXIUIConfig.shareInstance.getCachePath() + "/\(_attr.first!.hashHex(by: .MD5)).\(_attr.last ?? "")"
        }
        ...
        //不存在,下载到缓存
            ...
            CMRequest.shareInstance().downloadFileForServer(strUrl: _nUrl,
                ...
                if let _data = responseData as? Data {
                    if fm.createFile(atPath: _p, contents: _data) {
