...
struct CMRequest {
    ...
    func postRequestWithBodyFor(strUrl:String,
        ...
        if body != nil {
            debugPrint("请求地址：\(strUrl),参数：\(NSString.init(data: body!, encoding: String.Encoding.utf8.rawValue) ?? "暂无参数")")
        }
        else{
    ...
    private func setReturnValueFor(Data _d:Data?,
        ...
        if _d != nil {
            debugPrint("请求结果：" + (String.init(data: _d!, encoding: .utf8) ?? "--"))
        }
        else{
            debugPrint("请求结果：\(String(describing: jsonObj))")
        }
