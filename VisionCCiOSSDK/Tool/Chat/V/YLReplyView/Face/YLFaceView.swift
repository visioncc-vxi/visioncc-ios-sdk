...
class LoadDataViewController : UICollectionView {
    ...
    private lazy var btnRemove:UIButton = {
        ...
        btn.rx.safeTap.subscribe {[weak self] (_:Event<Void>) in
            guard let self = self else { return }
...
extension LoadDataViewController : UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if self.selectIndex > 0 {
            ...
            var _imgFace:YYAnimatedImageView? = cell.contentView.viewWithTag(1232) as? YYAnimatedImageView
            if _imgFace == nil {
                _imgFace = TGSUIModel.createAnimationImageFor(Rect: .zero,
                                                              andImage: nil)
                _imgFace?.tag = 1232
