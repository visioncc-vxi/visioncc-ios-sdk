...
class ChatCardChildCell : UICollectionViewCell {
    ...
    override func updateConstraints() {
        ...
        if self.contentView.subviews.contains(self.labCustomCardAmountName){
            self.labCustomCardAmountName.snp.makeConstraints({[weak self] make in
                guard let self = self else { return }
                make.left.equalTo(self.labName.snp.left)
                make.top.equalTo(self.labDescription.snp.bottom).offset(6)
                make.right.equalTo(self.labName.snp.right)
                make.height.equalTo(ChatCardChildCell.cell_ziduan_height)
            })
        }
    ...
    private func initView(){
        ...
        //价格
        self.contentView.addSubview(self.labCustomCardAmountName)
    ...
    private lazy var labProductPrice:YYLabel = {
        ...
        _lab.preferredMaxLayoutWidth = ChatCardChildCell.cell_description_max
    ...
    /// 金额描述(可选)
    private lazy var labCustomCardAmountName:YYLabel = {
        let _lab = TGSUIModel.createLable(rect: .zero,
                                          text: nil,
                                          textColor: .colorFromRGB(0x424242),
                                          font: UIFont.systemFont(ofSize: 12, weight: .regular),
...
extension ChatCardChildCell {
    ...
    static func getCardChildCellHeightFor(Data _data:MessageCustomItems?) -> CGFloat {
        ...
        //金额描述
        let _ccan:String? = _data?.customCardAmountName
        _data?.isWrap = false
        if _ccan != nil {
            var _w:CGFloat = _ccan!.yl_getWidthFor(Font: .systemFont(ofSize: 12, weight: .regular))
            //超过一行
            if _w > ChatCardChildCell.cell_description_max {
                _tempH += cell_ziduan_height + 6
                _data?.isWrap = true
            }
            else {
                let _unit = _data?.customCardAmountSymbol ?? "￥"//货币类型(单位)
                if let _p = _data?.customCardAmount as? String,_p.isEmpty == false {
                    ...
                    _w += YYTextLayout.init(containerSize: CGSize(width:ChatCardChildCell.cell_description_max,height: CGFloat(MAXFLOAT)),text: _attr)?.textBoundingSize.width ?? 0
                    _w += 2.5 //间距
                }
                
                if let _op = _data?.customCardOriginalAmount,_op.isEmpty == false,(_data?.customMenus?.count ?? 0) > 0 {
                    _w += (_op + _unit).yl_getWidthFor(Font: UIFont.systemFont(ofSize: 11, weight: .regular)) + 5
                    _w += 2.5
                }
    ...
    func cellBindValueFor(CellData _cd:MessageCustomItems?) {
        ...
        var _layout = YYTextLayout.init(containerSize: CGSize(width:ChatCardChildCell.cell_description_max,height: CGFloat(MAXFLOAT)),
                                        text: _attr)
        ...
        //金额描述
        if let _ccan = _cd?.customCardAmountName,_ccan.isEmpty == false {
            self.labCustomCardAmountName.isHidden = false
            self.labCustomCardAmountName.text = _ccan
            
            let _w:CGFloat = _ccan.yl_getWidthFor(Font: .systemFont(ofSize: 12, weight: .regular))
            self.labCustomCardAmountName.snp.remakeConstraints {[weak self] make in
                guard let self = self else { return }
                make.left.equalTo(self.labName.snp.left)
                make.top.equalTo(self.labDescription.snp.bottom).offset(6)
                make.height.equalTo(ChatCardChildCell.cell_ziduan_height)
            ...
            self.labCustomCardAmountName.isHidden = true
        }
        ...
        if let _p = _cd?.customCardAmount as? String,_p.isEmpty == false {
            ...
            _layout = YYTextLayout.init(containerSize: CGSize(width:ChatCardChildCell.cell_description_max,height: CGFloat(MAXFLOAT)),text: _attr)
            self.labProductPrice.textLayout = _layout
            
            if self.labCustomCardAmountName.isHidden == false {
                self.labProductPrice.snp.remakeConstraints {[weak self] make in
                    guard let self = self else { return }
                    make.left.equalTo(self.labCustomCardAmountName.snp.right).offset(2.5)
                    make.top.equalTo(self.labCustomCardAmountName.snp.top)
                    make.width.equalTo(_layout?.textBoundingSize.width ?? 55)
                    make.height.equalTo(22)
                }
            }
            else{
            ...
            self.labProductPrice.textLayout = nil
        }
        
        //[S]划线价格
        let _originalAmount:String? = _cd?.customCardOriginalAmount
        self.labOriginalPrice.text = _unit + (_originalAmount ?? "")
        debugPrint("labOriginalPrice:\(self.labOriginalPrice.text ?? "--")")
        if _originalAmount == nil || _originalAmount?.isEmpty == true {
            ...
            var _w:CGFloat = 60
            if let _cw = self.labOriginalPrice.text?.yl_getWidthFor(Font: self.labOriginalPrice.font!) {
                _w = _cw + 5
            }
            ...
            if self.labCustomCardAmountName.isHidden == false {
                if _cd?.isWrap == true {
                    self.labOriginalPrice.snp.remakeConstraints {[weak self] make in
                        ...
                        make.top.equalTo(self.labProductPrice.snp.bottom).offset(2.5)
                        make.width.equalTo(_w)
                        make.height.equalTo(22)
                    }
                }
                ...
                _lab.snp.makeConstraints {[weak self] make in
                    ...
                    if i == 0 {
                        if self.btnRight.isHidden == false {
                            ...
                            if self.labProductPrice.text == nil ||
                                self.labProductPrice.textLayout == nil || self.labProductPrice.text?.isEmpty == true {
