...
extension ChatImageCell {
    ...
    private func bindValueFor(Message m:MessageModel){
        ...
        self.messagePhotoImageView.subviews.first(where: { $0.tag == _svg_tag })?.removeFromSuperview()
        ...
        //线上图
            ...
            if _url.lowercased().hasSuffix(".svg") || _url.lowercased().contains(".svg?") {
                TGSUIModel.bindImageFor(Url: _url,
                                        andView: self.messagePhotoImageView,
                                        andViewSize: VXIUIConfig.shareInstance.faceImageSize())
                
                _tempBlock(TGSUIModel.pictureResetSizeFor(Image: nil,orOriginalSize: VXIUIConfig.shareInstance.faceImageSize()),nil,false)
            }
            else{
                ...
                self.messagePhotoImageView.yy_setImage(with: _newUrl,
                    ...
                    guard let self = self else { return }
                    if let _image = _img {
                        let _size = TGSUIModel.pictureResetSizeFor(Image: _image, orOriginalSize: nil)
                        _tempBlock(_size,nil,true)
