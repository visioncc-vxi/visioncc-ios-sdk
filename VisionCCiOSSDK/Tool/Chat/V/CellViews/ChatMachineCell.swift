...
extension ChatMachineListView: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        var cell:ChatMachineListViewCell? = tableView.dequeueReusableCell(withIdentifier: cellId) as? ChatMachineListViewCell
        if cell == nil {
            cell = ChatMachineListViewCell.init(style: .default, reuseIdentifier: cellId)
        }
...
class ChatMachineListViewCell: UITableViewCell {
    ...
    func updateData(item:MessageGroupItems?, isHiddenLine:Bool){
        showContentLabel.text = item?.title?.replacingOccurrences(of: "\n\n", with: "").replacingOccurrences(of: "\n", with: "")
