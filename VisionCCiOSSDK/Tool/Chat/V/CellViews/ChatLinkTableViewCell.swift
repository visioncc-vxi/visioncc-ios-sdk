...
class ChatLinkTableViewCell: BaseChatCell {
    ...
    override func updateMessage(_ m: MessageModel, idx: IndexPath) {
        ...
        /// 获取评价状态数据
        if m.mType == MessageBodyType.evaluat.rawValue {
            self.viewModel?.evaluatFeedbackDataPublishSubject.subscribe {[weak self] (_input:Event<Any>) in
                ...
                if _isOK,m.mId == _mid,let _r = _result as? EvaluatResultModel,(_r.options?.count ?? 0) > 0 {
                    //已评价，不可再次评价
                    self.message?.messageBody?.isEvaluated = true
                    _ = self.setEvaluatTxt(ByColor: .colorFromRGB(0x9BC2F8))
                }
...
extension ChatLinkTableViewCell {
    ...
    private func cellBindValue(){
        ...
        /// 满意度 评价
            ...
            //未评价，但已超时(不可评价)
            else if let _ct = m.createTime,let _vp = m.messageBody?.validPeriod {
                if (TGSUIModel.localUnixTimeDouble() - _ct > Double(_vp * 60 * 1000)) {
                    _link_Color = .colorFromRGB(0x9BC2F8)
                    m.messageBody?.isEvaluated = true
                    debugPrint("超时不可评价：{createTime:\(_ct),时长:\(_vp)M}")
                }
            }
            
            let _attr = self.setEvaluatTxt(ByColor: _link_Color)
            _h = max(21,self.labInfo.textLayout?.textBoundingSize.height ?? _attr.size().height)
        }
    ...
    private func setEvaluatTxt(ByColor _c:UIColor) -> NSAttributedString {
        let _full_String = message?.messageBody?.titleWord ?? message?.messageBody?.content ?? "点击这里给我评价"
        let _attr = TGSUIModel.setAttributeStringTexts(strFullText: _full_String,
                                                       andFullTextFont: VXIUIConfig.shareInstance.cellMessageFont(),
                                                       andFullTextColor: VXIUIConfig.shareInstance.cellMessageColor(),
                                                       withChangeText: [_full_String],
