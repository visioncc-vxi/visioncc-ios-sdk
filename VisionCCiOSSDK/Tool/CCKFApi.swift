    ...
    private func setUI(){
        ...
        VXISocketManager.share.viewModel.enterSectionPublishSubject.subscribe { (_input:Event<Any>) in
            ...
            if _isOK == false {
                ...
                print("enterSectionPublishSubject-进入会话成功")
                        ...
                        if _index == nil {
                            ...
                            //排序
                                ...
                                if let _index = self.dataArray.lastIndex(where: { _rct <= $0.createTime ?? .zero }) {
                                    ...
                                    debugPrint("会话消息已插入新增:{\(_index),mId:\(_result.mId ?? 0),createTime:\(_result.createTime ?? 0)，mType:\(_result.mType ?? 0),renderMemberType:\(_result.renderMemberType ?? 0),_result:\(_result)}")
                                }
                            }
                            lock.unlock()
                        }
